  ...
  return (
    <main className="min-h-screen bg-bg-dark text-text-inverse">
        ...
        <div className="mx-auto max-w-7xl px-6 pt-28 pb-10">
          ...
          >
            ...
            <p className="mt-6 text-text-inverse/70 text-base sm:text-lg leading-relaxed">
              Explore Megica Group’s manufacturing strength, quality systems,
              export readiness, and real-world applications — presented in
              curated albums.
            </p>
          </motion.div>
      ...
      <Lightbox
        ...
        brandLabel="Megica Group Gallery"
      />
    </main>
  );
}
...
function ParallaxRow({ x, items, onOpen }) {
  return (
    <motion.div
      style={{ x }}
      className="relative w-[100vw] scrollbar-hide overflow-x-scroll"
    >
...
function Lightbox({ open, item, onClose, brandLabel = "Megica Gallery" }) {
  useEffect(() => {
    if (!open) return;
